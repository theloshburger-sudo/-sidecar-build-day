...
| **Draw anything** | A free drawing canvas (circles, arcs with arrows, lines, shapes, dots, labels) for any picture a student asks for: a clock for powers of i, a pizza for fractions, a floor plan, a cell. Teacher is told to draw exactly what was asked, never swap in a graph. |
| **Words ↔ strokes** | Each spoken line gets a number badge ①②③. The same number appears on the board next to what that line drew, and the strokes glow while that line is spoken. |
| **Real math notation** | Exponents and subscripts (i¹⁴², x^2, H₂O) render as proper raised/lowered text on the board and in the app, and are read aloud as "to the power of". |
